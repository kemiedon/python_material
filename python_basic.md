...
- 可在 VS Code 中使用「右上角▶️執行」或「右鍵 → Run Python File」直接執行
...
**`<font style='color: brown;'>`範例檔參考: 01.Basic/01_variables_naming.py`</font>`**
...
**`<font style='color: brown;'>`範例檔參考: 01.Basic/02_data_types.py`</font>`**

### 數值運算處理

### 算術運算子與邏輯運算子

算術運算子用於執行基本的數學計算，而邏輯運算子用於組合關係運算的結果（True 或 False），以進行更複雜的條件判斷，通常在if或elif中使用。
...
**`<font style='color: darkblue;'>`開啟cost_exercise.py，完成下列練習題`</font>`**
:::info
...
**`<font style='color: brown;'>`範例檔參考: 01.Basic/cost_example.py`</font>`**
...
**`<font style='color: brown;'>`範例檔參考: 01.Basic/04_type_conversion_precedence.py`</font>`**
...
比較與邏輯運算子與if經常搭配使用，常用於組合多個條件來進行複雜的決策，例如：年齡與收入門檻 (age >= 18 and income >= 30000)。

**`<font style='color: brown;'>`範例檔參考: 01.Basic/05_conditional_statements.py`</font>`**

### 隨堂練習

`<font style="color: darkblue;">`**在01.Basic/practice資料夾中新增if_exercise.py，完成下列練習**
`</font>`
:::info
...
**range(開始, 結束, 間隔)：從 0 起，每次+2，到11前結束**
...
break 和 continue是迴圈中的「**緊急按鈕**」，讓你可以精準控制迴圈的執行進度。
...
`<font style="color: darkblue;">`**在01.Basic/homework資料夾中, 完成下列練習題**
`</font>`
...
- Python遇到錯誤時，會「拋出」例外，並顯示**錯誤訊息**，協助我們判斷錯誤位置與原因。
...
| NameError         | 變數未定義或拼錯名稱         | `print(a)` (a未宣告過)     |
...
| IndentationError  | 縮排錯誤                     | if語句後未縮排               |
...
1. **讀懂錯誤訊息**：看紅字提示的錯誤類型（如NameError、IndexError），注意「最後一行」的錯誤類型及Traceback位置。
...
4. **使用print()/偵錯工具追蹤**：在關鍵步驟前後加print看變數內容，或用VS Code等IDE的「中斷點」、「逐行執行」功能協助追查。
...
- 練習自己「**刻意**」打出錯誤，觀察Python錯誤訊息。
- 多讀多理解Traceback訊息，並寫下遇過的錯誤與原因。
- 練習用try-except包覆input、檔案開啟等「易發生錯誤」的動作。
- 嘗試自行設計錯誤處理流程，增強Debug能力。
...
`<font style="color: darkblue;">`**在01.Basic/practice/string_slicing_practice.py, 完成下列練習題**
`</font>`
...
`<font style="color: darkblue;">`**開啟01.Basic/string_exercise.py，完成下列練習**`</font>`
